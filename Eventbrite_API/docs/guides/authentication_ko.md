...
이 가이드에서는 Eventbrite 개발자 대시보드에서 토큰을 생성하고,
...
하며, 토큰이 없으면 API는 `401 Unauthorized` 오류를 반환합니다.

Eventbrite는 두 가지 인증 자격 증명을 제공합니다.

| 자격 증명 | 설명 | 사용 시점 |
| --- | --- | --- |
| 프라이빗 토큰(Private Token) | 장기간 유효하며, 개인이 접근 가능한 토큰 | 테스트, 서버 사이드 스크립트 |
...
**사전 요구사항:** Eventbrite 계정. 아직 없다면
[여기서 만드세요](https://www.eventbrite.com/signin/).
...
`YOUR_PRIVATE_TOKEN`을 앞 단계에서 복사한 토큰으로 바꾸세요.

**쿼리 파라미터 방식 (비권장):** URL 파라미터로 토큰을 전달할 수도 있습니다.
URL에 포함된 토큰은 서버와 브라우저 로그에 기록되므로
보안 위험이 생길 수 있어서 프로덕션 환경에서는 사용하지 마세요.
...
Eventbrite는 인증 코드를 포함해서 사용자를 리디렉션 URI로 돌려보냅니다.
