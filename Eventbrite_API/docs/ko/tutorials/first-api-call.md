...
Eventbrite API로 실제 요청을 보내고 응답을 확인하면서 API 작동 방식을
배워보세요. 이 튜토리얼은 Eventbrite API 콘솔과 Postman을 사용합니다.
코드 에디터는 필요하지 않습니다.
...
[인증 가이드](../guides/authentication.md)를 먼저 따라한 후 여기로 돌아오세요
...
알림처럼 일반 문자열이 필요한 곳에는 `text`를, 웹 인터페이스에서 렌더링할
때는 `html`을 쓰세요. 이 패턴에 대한 자세한 내용은
...
4. 원하면 컬렉션 이름을 알아보기 쉽게 바꾸세요. 예: `Eventbrite API`
