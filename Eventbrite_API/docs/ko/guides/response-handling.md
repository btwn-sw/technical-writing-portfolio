...
Eventbrite API 응답은 일관된 패턴을 따릅니다. 
이 가이드를 통해 응답 구조가 왜 이렇게 설계됐는지 이해하고, 이벤트 데이터를 올바르게 읽고, 누락된 필드와 HTML 콘텐츠를 안전하게 처리할 수 있습니다.

엔드포인트 스키마와 파라미터 정의는 [API 레퍼런스](../api/api-reference.md)를 참고하세요.
...
이벤트 객체
├── 식별          id, url
├── 텍스트 콘텐츠  name, description            (각각 text / html 형식 포함)
...
`text`를 사용하고, 웹 인터페이스에서 렌더링할 때는 `html`을 사용합니다.
...
`error_description`은 로깅과 디버깅에 사용하세요. 최종 사용자에게 그대로
표시하는 용도로는 적합하지 않습니다.
